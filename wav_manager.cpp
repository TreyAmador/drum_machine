...
int WAVManager::create_buffer_size(WAV* wav, RhythmAttributes& rhythmicity) {
	return 
		static_cast<int>(BYTES_PER_BIT * 
			static_cast<float>(*wav->header_->bits_per_sample_) * 
			static_cast<float>(*wav->header_->sample_rate_) * 
			SECONDS_PER_MINUTE * (1.0f / rhythmicity.tempo_) * 
			static_cast<float>(rhythmicity.beats_per_measure_) * 
			static_cast<float>(rhythmicity.total_measures_) * 
			static_cast<float>(*wav->header_->num_channels_));
}
